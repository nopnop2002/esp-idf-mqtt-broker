        ...
        switch (mm->cmd) {
            ...
            case MQTT_CMD_PUBLISH: {
                ...
                if (isascii(mm->data.ptr[0])) {
                    ...
                    oled_invert_printf("pub [%.*s]", (int) mm->topic.len, mm->topic.ptr);
...
void mqtt_server(void *pvParameters)
{
    ...
    oled_invert_printf("%s start", pcTaskGetName(NULL));
